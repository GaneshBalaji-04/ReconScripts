...
def assetfinder(domain):
    global assetfinder_file  # Need to declare as global to modify the global variable
...
def subfinder(domain):
    global subfinder_file  # Need to declare as global to modify the global variable
    subdomain_set = {domain}
    processed_domains = set()
    ...
    while subdomain_set:
        ...
        if current_domain in processed_domains:
            continue
        os.system(f"subfinder -d {current_domain} -silent >> {directory}")  # Fixed typo: curent_domain -> current_domain
        processed_domains.add(current_domain)
